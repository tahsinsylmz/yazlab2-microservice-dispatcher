...
```
## 6.1 Dispatcher Routing Algoritması

Dispatcher, gelen isteğin URL path bilgisini analiz ederek ilgili mikroservise yönlendirme yapacaktır.

Algoritma Adımları:

1. Gelen isteğin path bilgisi alınır.
2. Path prefix kontrol edilir.
3. İlgili servis belirlenir.
4. Yetki kontrolü yapılır.
5. İstek hedef servise iletilir.
6. Yanıt client’a döndürülür.

```mermaid
flowchart TD
    A[Request Received] --> B{Check Path}
    B -->|/users| C[User Service]
    B -->|/orders| D[Order Service]
    B -->|/messages| E[Message Service]
    C --> F[Return Response]
    D --> F
    E --> F
```
...
## 9.1 Dispatcher Sınıf Yapısı

Dispatcher servisi aşağıdaki temel bileşenlerden oluşacaktır:
